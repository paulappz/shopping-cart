...
angular
	...
	.controller('ShoppingCartCtrl', ['$scope', function($scope) {
		...
		$scope.products = [
			{id: 0, price: 10.50, name: "Sneakers", photoUrl:"./img/sneaker.jpg/", availableQty: 4},
		...
		$scope.groupItems = function(cartItems, isZeroQty=false) {
			var retVal = {}; // initialize empty object
			
			if (cartItems.length === 0)
				return retVal;
			
			cartItems.forEach(function(item, index) {
				retVal[item.id] = (retVal[item.id] || {qty: 0, price:item.price, photoUrl:item.photoUrl,
